...
##      9.When verify_check_digit is True display the school and grade. If false display error
## 
## ERROR HANDLING:
...
def get_check_digit(idnumber: str) -> int:
    ...
    total = 0
    for i in range(0, 5):
        total += ((i + 1) * character_value(
            idnumber[i]))  # gets the values and indexs and times them togther, the adds them to the total
    for i in range(5, 10):
...
def verify_check_digit(idnumber: str) -> tuple:
    ...
    elif idnumber[6] != '1' and idnumber[6] != '2' and idnumber[6] != '3' and idnumber[
        6] != '4':  # checks that index 6 is 1 to 4
        return False, 'The seventh character must be 1 2 3 or 4'
    elif int(idnumber[9]) != get_check_digit(
            idnumber):  # checks if the entered last digit matches the calculated last digit
